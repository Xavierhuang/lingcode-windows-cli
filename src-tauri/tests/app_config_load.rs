...
use lingcode_cli_lib::{AppError, MultiAppConfig};
...
fn load_valid_v2_config_succeeds() {
    ...
    assert!(loaded
        .get_manager(&lingcode_cli_lib::AppType::Claude)
        .is_some());
    assert!(loaded.get_manager(&lingcode_cli_lib::AppType::Codex).is_some());
}
